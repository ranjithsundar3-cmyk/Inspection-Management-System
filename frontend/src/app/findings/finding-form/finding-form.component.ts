          ...
          <form (ngSubmit)="onSubmit()" class="app-form">
            ...
            <div class="form-row">
              <div class="form-group">
                ...
                <input type="number" [(ngModel)]="finding.inspectionId" name="inspectionId" required>
              </div>
              <div class="form-group">
            ...
            <div class="form-group">
              ...
              <textarea [(ngModel)]="finding.evidence" name="evidence" rows="2" placeholder="Evidence"></textarea>
            </div>
...
export class FindingFormComponent implements OnInit {
  finding: Finding = {
    ...
    severity: 'medium', status: 'open', category: 'operational',
    location: '', dueDate: null, resolvedDate: null, correctiveAction: '',
    preventiveAction: '', rootCause: '', verified: false, verifiedById: 0,
    assignedToId: 0, inspectionId: 0, reportedById: 0,
  ...
  ngOnInit() {
    ...
    if (inspectionId) {
      this.finding.inspectionId = Number(inspectionId);
    }
  }

  onSubmit() {
    if (!this.finding.title || !this.finding.inspectionId) {
      alert('Title and Inspection ID are required'); return;
    }
    this.saving = true;
    this.findingService.create(this.finding).subscribe({
      next: () => { this.router.navigate(['/findings']); },
      error: (err) => { alert(err.error?.error || 'Failed to create finding'); this.saving = false; }
    });
  }

  cancel() { this.router.navigate(['/findings']); }
}