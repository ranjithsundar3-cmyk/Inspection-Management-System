...
@Component({
  ...
  template: `
    <div class="page-header">
      <h2>Findings</h2>
      <button mat-raised-button color="primary" (click)="navigate('/findings/new')">
        <span class="material-icons">add</span>New Finding
      </button>
    </div>
    ...
    <div *ngIf="!loading" class="card-grid">
      <mat-card *ngFor="let item of findings" class="item-card" (click)="navigate('/findings/' + item.id)">
        <mat-card-content>
          <div class="badges">
...
export class FindingListComponent implements OnInit {
  ...
  loadFindings() {
    this.findingService.getAll().subscribe({
      next: (data: PageResponse<Finding>) => { this.findings = data.data; this.loading = false; },
